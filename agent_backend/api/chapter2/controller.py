"""
챕터 2 컨트롤러 — LCEL 체인 REST 라우터. (구 api/llm.py)
체인을 여기서 중복 정의하지 않고 service.py 의 빌더를 재사용한다.
...
from agent_backend.api.chapter2 import dto
from agent_backend.api.chapter2.service import (
    build_explain,
    build_chat,
    build_sentiment,
    build_analyze,
...
@router.post("/explain", response_model=dto.TextResult)
def explain(body: dto.TopicIn):
    """2-2. LCEL: 주제를 3문장으로 설명."""
    logger.info("explain 요청: topic=%r", body.topic)
    return {"result": _explain().invoke({"topic": body.topic})}


@router.post("/chat", response_model=dto.TextResult)
def chat(body: dto.QuestionIn):
    """2-3. 역할(강사) 프롬프트로 답변."""
    logger.info("chat 요청: question=%r", body.question)
    return {"result": _chat().invoke({"question": body.question})}


@router.post("/sentiment", response_model=dto.DictResult)
def sentiment(body: dto.ReviewIn):
...
@router.post("/analyze", response_model=dto.DictResult)
def analyze(body: dto.TextIn):
